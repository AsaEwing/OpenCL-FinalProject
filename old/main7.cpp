...
void print_Out2(int sizeXY,int count, int indexArg, uchar** orsArray, double** outArray){
    ...
    for (int y=0;y<sizeXY;y++){
        for (int x=0;x<sizeXY;x++){
            ...
            if (tmp[y][x]>maxErr) maxErr=tmp[y][x];
            if (tmp[y][x]<minErr) minErr=tmp[y][x];
            err+=pow(tmp[y][x],2);
        }
        //cout << endl;
    }
    ...
    if (indexArg==7){
        ...
        for (int y=0;y<sizeXY;y++){
            for (int x=0;x<sizeXY;x++){
                if (outArray[y][x]>maxErr2) maxOut=outArray[y][x];
                if (outArray[y][x]<minErr2) minOut=outArray[y][x];
            }
        }
        ...
        for (int y=0;y<sizeXY;y++){
            for (int x=0;x<sizeXY;x++){
                if (outArray[y][x]<0){
                    tmpOut[y][x]=0;
                } else if (outArray[y][x]>28){
                    tmpOut[y][x]=28;
                } else {
                    tmpOut[y][x]=outArray[y][x];
                }
        ...
        for (int y=0;y<sizeXY;y++){
            for (int x=0;x<sizeXY;x++){
                ...
                tmp2[y][x]=tmpOut[y][x]-orsArray[y][x];
                if (tmp2[y][x]>maxErr2) maxErr2=tmp2[y][x];
                if (tmp2[y][x]<minErr2) minErr2=tmp2[y][x];
                err2+=pow(tmp2[y][x],2);
            }
            //cout << endl;
        }
    ...
    if (indexArg==0 || indexArg==2){
        ...
        for (int ii=0;ii<sizeXY;ii++){
            ...
            for (int jj=0;jj<sizeXY;jj++){
                if (indexArg==0) tmp[ii]+=outArray[jj][ii];
                else if (indexArg==2) tmp[ii]+=outArray[count-1-ii][jj];
            }
        }
        ...
        for (int ii=0;ii<sizeXY;ii++){
            tmp[ii]=0;
            for (int jj=0;jj<sizeXY;jj++){
                if (indexArg==4) tmp[ii]+=outArray[jj][sizeXY-1-ii];
                else if (indexArg==6) tmp[ii]+=outArray[ii][jj];
            }
        }
